...
LOCAL_VERSION=`cat build.gradle.txt | grep "ext.http4k_version" | sed -E "s/.*\'(.*)\'/\1/g"`
...
sed -i '' "s/.*ext.http4k_version = '.*'/    ext.http4k_version = '$BINTRAY_VERSION'/g" build.gradle.txt

git add build.gradle.txt
